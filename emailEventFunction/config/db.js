...
const premConnectDbs = {};

module.exports = {
  getDb: async () => {
    if (!client || new Date() > new Date(credentialsExpiration)) {
      console.log("Creating new db connection...", localDatabase);
  ...
  isolatedDatabase: async (domain) => {
    !client || new Date() > new Date(credentialsExpiration)
      ? await connectToServer()
      : undefined;
    if (!(`${localDatabase}-${domain}` in premConnectDbs)) {
      console.log(
        `Premium database connection not found so connecting to: ${localDatabase}-${domain}`
      );
      const connection = client.db(`${localDatabase}-${domain}`);
      console.log("Assigning db connection to premium db object");
      Object.assign(premConnectDbs, {
        [`${localDatabase}-${domain}`]: connection,
      });
      return connection;
    } else {
...
const connectToServer = async () => {
  if (server && server === "local") {
    ...
    const { AccessKeyId, SessionToken, SecretAccessKey, Expiration } =
      Credentials;
