...
Given('the CSV file contains valid devices data', async () => {
    emailStub = sinon.stub(emailHandler, 'sendEmailAttachments').resolves({});
    const readThingCsv = await fs.readFileSync('./testFiles/device.csv')
...
When('the Lambda function is triggered by the file upload event for devices', async () => {
    type = 'devices';
...
Then('the system should parse the file correctly for devices', async () =>  {
    response = await processS3File(bucketName, key, data, type, customerId, fileName);
})

Then('the devices should be added to the database for devices', function () {
    expect(response).to.be.undefined;
})

Then('an email should be sent with the import status for devices', function () {
