...
Given('the CSV file contains valid locations data', async () => {
    emailStub = sinon.stub(emailHandler, 'sendEmailAttachments').resolves({});
    const readThingCsv = await fs.readFileSync('./testFiles/location.csv')
...
When('the Lambda function is triggered by the file upload event for locations', async () => {
    type = 'locations';
...
Then('the system should parse the file correctly for locations', async () =>  {
    response = await processS3File(bucketName, key, data, type, customerId, fileName);
})

Then('the locations should be added to the database for locations', function () {
    expect(response).to.be.undefined;
})

Then('an email should be sent with the import status for locations', function () {
