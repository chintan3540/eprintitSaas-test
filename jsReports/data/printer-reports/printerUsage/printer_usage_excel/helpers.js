...
var totalOrientationPagesCount = 0;
var totalStapledPagesCount = 0;
...
function totalOrientaionPages(data, decimal, decimalSeparator) {
    var sum = 0;
    data.forEach(function(i) {
        sum += +i.TotalOrientation;
    })
    totalOrientationPagesCount = sum;
    var totalOrientation = hasDecimal(totalOrientationPagesCount, decimalSeparator)
    return totalOrientation;
}


function totalStapledPages(data, decimal, decimalSeparator) {
    var sum = 0;
    data.forEach(function(i) {
        sum += +i.TotalStapled;
    })
    totalStapledPagesCount = sum;
    var totalStapled = hasDecimal(totalStapledPagesCount, decimalSeparator)
    return totalStapled;
}
