...
const { MongoDB, localDatabase, awsAccountNumber, server,
  testSecretsDb, testSecretsDatabse, region} = require('./config')
...
module.exports = {
  ...
  getTestSecretsDb: async () => {
    const secretClient = new MongoClient(testSecretsDb)
    await secretClient.connect()
    return secretClient.db(testSecretsDatabse)
  }
}
